...
const Step2GuidanceTopics = forwardRef<StepRefType>((_, ref) => {
  ...
  const validateAndSubmit = async (): Promise<boolean> => {
    const newErrors = children.map((child) => !child.topics || child.topics.length === 0);
    setErrors(newErrors);
...
//Dynamic Area Of interest 
  ...
  useImperativeHandle(ref, () => ({
    validateAndSubmit,
    getValues: () => ({
      topics: children.map((child) => child.topics || []),
    }),
  }));

  const handleTopicToggle = (idx: number, topic: string) => {
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-top sm:justify-between gap-4">
        <div className="flex gap-3 items-center justify-center">
          ...
          <div>
            <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-alice-black mb-[5px]">Topics of Guidance</h2>
            <p className="text-sm text-alice-darkgray font-normal">Select all areas where you’d like personalized support</p>
          </div>
        </div>
      </div>
