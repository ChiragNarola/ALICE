...
const defaultChildTopics = {
  topics: [] as string[],
};

const Step2GuidanceTopics: React.FC = () => {
  const [children, setChildren] = useState([
    { ...defaultChildTopics },
    { ...defaultChildTopics }, // Example: 2 children for demo; in real use, sync with step 1
  ]);

  const handleTopicToggle = (childIdx: number, topic: string) => {
    setChildren((prev) =>
      prev.map((child, idx) =>
        idx === childIdx
          ? {
            ...child,
            topics: child.topics.includes(topic)
              ? child.topics.filter((t) => t !== topic)
              : [...child.topics, topic],
          }
          : child
      )
    ...
    <div>
      <div className="flex flex-col sm:flex-row sm:items-top sm:justify-between gap-4">
        <div className="flex gap-3 items-center justify-center">
          ...
          <div>
            <h2 className="text-lg sm:text-xl md:text-2xl font-bold text-alice-black mb-[5px]">Topics of Guidence</h2>
            <p className="text-sm text-alice-darkgray font-normal">Select all area where you’d like personalized support</p>
          </div>
        </div>
      </div>
      <hr className="my-4 md:my-6 border-alice-gray" />
      {children.map((child, idx) => (
        <div key={idx} className="mb-4 lg:mb-6 border border-alice-gray rounded-xl p-4 lg:p-6 relative bg-white w-full">
          <div className="absolute top-4 right-4">
            <button type="button" onClick={() => deleteChild(idx)} className="bg-[#F00044] text-white font-semibold flex items-center gap-1 px-2 py-2 sm:px-[10px] sm:py-[5px] rounded-md hover:bg-[#e6002e] transition-colors text-sm">
            <svg width="18" height="18" className="sm:me-[6px]" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <div className="mb-3 font-bold text-base sm:text-lg lg:text-xl">{idx + 1}{idx === 0 ? 'st' : idx === 1 ? 'nd' : idx === 2 ? 'rd' : 'th'} Child Information</div>
          <div className="mb-2 font-semibold">Gender</div>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 xl:gap-6">
            {TOPICS.map((topic) => {
              const checked = child.topics.includes(topic);
              return (
                <label
                  key={topic}
                  className={`flex items-center px-3 xl:px-5 py-[14px] lg:py-[18px] rounded-xl border border-alice-gray transition-all cursor-pointer select-none text-[15px] md:text-base font-normal
                    ${checked ? 'bg-alice-teal text-white border-alice-teal' : 'bg-white text-alice-darkgray border-[#E5E5E5] hover:border-alice-teal'}`}
                >
                  <input
                    type="checkbox"
                    checked={checked}
