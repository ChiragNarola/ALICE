...
const defaultChildConcerns = {
  concerns: [] as string[],
};

const Step3CurrentConcerns: React.FC = () => {
  const [children, setChildren] = useState([
    { ...defaultChildConcerns },
    { ...defaultChildConcerns }, // Example: 2 children for demo; in real use, sync with previous steps
  ]);

  const handleConcernToggle = (childIdx: number, concern: string) => {
    setChildren((prev) =>
      prev.map((child, idx) =>
        idx === childIdx
          ? {
              ...child,
              concerns: child.concerns.includes(concern)
                ? child.concerns.filter((c) => c !== concern)
                : [...child.concerns, concern],
            }
          : child
      )
    );
      ...
      <div className="flex flex-col sm:flex-row sm:items-top sm:justify-between gap-4">
        <div className="flex gap-3 items-center justify-center">
          ...
          <div>
            ...
            <p className="text-sm text-alice-darkgray font-normal">Select any current concerns (optional)</p>
          </div>
        </div>
      </div>
      <hr className="my-4 md:my-6 border-alice-gray" />
      {children.map((child, idx) => (
        <div key={idx} className="mb-4 lg:mb-6 border border-alice-gray rounded-[16px] p-4 lg:p-6 bg-white relative">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 xl:gap-6">
            {CONCERNS.map((concern) => {
              const checked = child.concerns.includes(concern);
              return (
                <label
                  key={concern}
                ...
                >
                  <input
                    type="checkbox"
                    checked={checked}
                    onChange={() => handleConcernToggle(idx, concern)}
