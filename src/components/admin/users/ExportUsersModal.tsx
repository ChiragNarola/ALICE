...
export default function ExportUsersModal({ isOpen, onClose }: ExportUsersModalProps) {
    ...
    const handleExport = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (startDate) queryParams.start_date;
            if (endDate) queryParams.end_data;
            const blob = await exportUsers(queryParams);
                        ...
                        >
                            <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                                ...
                                <form onSubmit={handleExport} className="space-y-4">
                                    <div className={`flex items-center gap-3 mt-2 p-2 rounded-md transition${disableCalender ? "bg-gray-50" : ""}`}>
                                        <label className="flex items-center gap-3 cursor-pointer text-sm text-gray-700 select-none">
                                            <input
                                                type="checkbox"
                                                checked={disableCalender}
                                                onChange={(e) => {
                                                    const checked = e.target.checked;
                                                    setDisableCalender(checked);
                                                ...
                                                className="h-4 w-4 rounded border-gray-300 text-[#134e4a] focus:ring-[#134e4a]"
                                            />
                                            Download all users from beginning
                                        </label>
                                    </div>
